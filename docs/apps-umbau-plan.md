...
- ~~Phase 8, übrige Punkte~~ — alle erledigt 2026-09-16 (apps.rautaki.ch, n8n, Vercel durch Claude; Netlify, Umami durch den Auftraggeber).
...
| n8n (ausserhalb des Repos) | Workflow, der Lab-Anmeldungen ins CRM Salesflare weiterleitet; Chatbot-Systemprompt im Workflow `Rautaki-Support`; dazu (im Plan zuerst übersehen) «Lab Nurture (Tag 2/6/12)» | Phase 8 — alle drei erledigt 2026-09-16 |
...
- [x] n8n: Workflow «Lab-Anmeldung → Salesflare» deaktivieren oder löschen — **deaktiviert 2026-09-16, 11:31 Uhr** (Claude, Freigabe des Auftraggebers): «Lab-Registrierung → Salesflare» (`XuMhS4bxZphmkUFu`, Webhook-Empfänger der gelöschten Route). Zusätzlich gefunden und auf Entscheid des Auftraggebers **sofort deaktiviert (11:34 Uhr)**: «Lab Nurture (Tag 2/6/12)» (`EkzQFhQQmgRV3Hqt`, täglich 08:00, Mails an Lab-Kontakte über `/api/nurture-send`); angefangene Sequenzen brechen damit ab. Beide nur inaktiv, nicht gelöscht. Nebenbefund: `src/app/api/nurture-send/route.ts` in `rautaki-web` hatte damit keinen Aufrufer mehr — auf Entscheid des Auftraggebers entfernt (samt `src/lib/nurture-templates.ts`; `/api/unsubscribe` bleibt für bereits versandte Mails und referral-action)
- [x] Vercel, Projekt `rautaki-web`: Umgebungsvariable `N8N_LAB_WEBHOOK_URL` entfernen — **erledigt 2026-09-16, 11:29 Uhr** (Production und Preview, zwei Einträge); kein Code liest sie mehr, kein Redeploy nötig
- [x] Netlify: offene Formularanfragen der alten Demo prüfen, Site `rautaki-apps.netlify.app` löschen, Konto schliessen, falls sonst nichts darauf liegt — erledigt durch den Auftraggeber, 2026-09-16 (gelöscht)
- [x] Umami Cloud: Website der alten Demo löschen, Konto schliessen — erledigt durch den Auftraggeber, 2026-09-16 (gelöscht)
- Status: erledigt (2026-09-16)
- Status: offen
...
- 2026-09-16 — Englische Fassung von apps.rautaki.ch unter `/en` (Aufgabe 170 in app-schaufenster; Rahmen, Einleitungen, Impressum, Datenschutz — die Apps selbst bleiben deutsch). In `rautaki-web`: Menü und Fusszeile der englischen Seite, `llms.txt` (EN) und `llms-full.txt` (EN) zeigen auf `apps.rautaki.ch/en`. Erst nach dem Livegang von `/en` mergen.
- 2026-09-16 — Nachtrag Englische Fassung: Texte vom Auftraggeber freigegeben; Sitemap für apps.rautaki.ch (Aufgabe 171 in app-schaufenster, `/sitemap.xml` mit hreflang, in `robots.txt` genannt). Chatbot-Prompt nennt `apps.rautaki.ch/en` mit englischen App-Namen (LANGUAGE RULES, APPS, WEITERE SEITEN; Kontrollfrage 11), Freigabe des Auftraggebers 2026-09-16; eingespielt 12:07 Uhr nach dem Livegang von `/en` (Version `2595d941…`, drei Schritte, Kontrollfragen 1–4, 6, 7, 9–11 bestanden; Abweichungen bei 5 und 8 im Hardening-Dokument §5).
- 2026-09-16 — Chatbot-Prompt, Regeln zur App-Beschreibung (Freigabe des Auftraggebers): wörtliche App-Sätze, Listen nur mit Namen oder vollen Sätzen, «Die Apps sind auf Deutsch.» in jeder Antwort zu den Apps, EU AI Act Checker nie als Teil von apps.rautaki.ch. Eingespielt 12:16/12:18 Uhr (Version `ccbb00f3…`); 17 Kontrollläufe, 16 bestanden, einmal fehlte der Deutsch-Hinweis bei «Was ist das Lab?» (Hardening-Dokument §5). Nebenbei PR #110 (js-yaml-Untergrenzen) nach Prüfung gemergt.
