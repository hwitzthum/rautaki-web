...
const labToolSchemas = tools.map((tool) => ({
  ...
  "@type": "WebApplication",
  "@id": `https://www.rautaki.ch${tool.href}`,
  name: tool.title,
  url: `https://www.rautaki.ch${tool.href}`,
  ...
  return (
    <>
      <JsonLd
        schema={[
          ...labToolSchemas,
          breadcrumbSchema([{ name: "Lab", path: "/lab" }]),
        ]}
      />
      <LabGate>
        <section className="bg-cream border-t-3 border-gold px-6 sm:px-10 lg:px-20 pt-16 pb-24">
          <div className="mx-auto max-w-content">
            ...
            <h1 className="font-serif text-h2 lg:text-h1 font-normal leading-heading tracking-tight text-ink mb-4">
              Werkzeuge zum{" "}
              <span className="italic text-gold">Selberbauen</span>.
            </h1>
            <p className="max-w-reading font-ui text-body font-light leading-body text-ink/65 md:text-mid-grey mb-10">
              Interaktive Tools, die direkt im Browser laufen — kein Account,
              kein Server. Fortschritt als HTML- oder Word-Datei herunterladbar
              — zum späteren Weiterbearbeiten oder Teilen.
            </p>
            ...
            <ScrollReveal delay={tools.length * 90}>
              <div className="mt-[2px] bg-cream border border-dashed border-ink/15 p-7 lg:p-9">
                ...
                <p className="font-ui text-sm font-light leading-body text-mid-grey max-w-reading">
                  Prompt-Bibliotheken, KI-Readiness-Assessments und weitere
                  Tools. Haben Sie eine Idee?{" "}
                  <a
